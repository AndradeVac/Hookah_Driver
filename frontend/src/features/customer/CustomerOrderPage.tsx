...
type Step = 'register' | 'menu' | 'cart' | 'customer' | 'payment' | 'success'
...
export function CustomerOrderPage() {
  ...
  if (step === 'register') return <main className="customer-app"><header className="customer-header"><div><span className="customer-kicker">HOOKAH DRIVER</span><h1>Antes de começar</h1><p>Identifique seu pedido em poucos segundos.</p></div><div className="customer-qr"><QrCode size={20} /><span>QR do lounge</span></div></header><section className="customer-panel customer-register"><h2>Como podemos te chamar?</h2><label>Nome<input placeholder="João Vitor" value={name} onChange={(event) => setName(event.target.value)} /></label><label>WhatsApp<input placeholder="(11) 99999-9999" value={phone} onChange={(event) => setPhone(event.target.value)} /></label><label>Mesa<input placeholder="Ex.: 12" inputMode="numeric" value={tableNumber} onChange={(event) => setTableNumber(event.target.value)} /></label><small>Usaremos esses dados apenas para identificar e acompanhar seu pedido.</small><button className="customer-primary" type="button" disabled={!name.trim() || !phone.trim()} onClick={() => setStep('menu')}>Continuar <ArrowRight size={16} /></button></section></main>
