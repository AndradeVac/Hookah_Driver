...
const getFlavorImage = (flavorName: string): string => {
  const normalized = flavorName.toLowerCase().replace(/\s+/g, '-').replace(/[àáãäâ]/g, 'a').replace(/[èéêë]/g, 'e').replace(/[ìíîï]/g, 'i').replace(/[òóôõö]/g, 'o').replace(/[ùúûü]/g, 'u').replace(/ç/g, 'c')
  return `/images/essencias/ziggy-${normalized}.png`
}
const imageFor = (label: string, isFlavor?: boolean) => isFlavor ? getFlavorImage(label) : /Rosh|sabor|fruta|mel|uva|banana/i.test(label) ? fruitImage : /bebida|água|suco|guaraná|pepsi/i.test(label) ? loungeImage : foodImage
