...
export function CustomerJourneyPage() {
  ...
  return <main className="customer-app">{header}{error && <div className="customer-error">{error}</div>}
    ...
    {step === 'menu' && <section className="journey-menu"><label className="customer-search"><Search size={16} /><input placeholder="Buscar produtos" value={search} onChange={(event) => setSearch(event.target.value)} /></label><div className="journey-feature"><img src={PLACEHOLDER_IMAGE} alt="Rosh com frutas" /><div><span>Mais pedidos</span><h2>Rosh</h2><p>Escolha uma marca e descubra os sabores.</p><button onClick={() => setStep('brands')}>Escolher Rosh <ArrowRight size={16} /></button></div></div>{lastOrder.length > 0 && <button className="journey-reorder" onClick={() => { setCart(lastOrder); setStep('cart') }}><History size={16} /><span>Repetir último pedido</span><RotateCcw size={15} /></button>}<h3>Categorias</h3><div className="journey-categories">{categories.filter((item) => !['rosh', 'essências'].includes(item.name.toLowerCase())).map((category) => <button key={category.id} onClick={() => { setSelectedCategory(category); setStep('category') }}><img src={PLACEHOLDER_IMAGE} alt="" /><span>{category.name}</span><ChevronRight size={16} /></button>)}</div></section>}
