...
export function CustomerJourneyPage() {
  ...
  useEffect(() => {
    ...
    getPublicOrder(token).then((tracking) => {
      setOrder({ order_id: token, order_number: tracking.order_number, status: tracking.status, total: tracking.total, public_token: token, payment_status: tracking.payment_status, checkout_url: null })
  ...
  async function submit() { if (!name || !phone || !cart.length) return; setSending(true); try { const created = await createPublicOrder({ customer_name: name, customer_phone: phone, payment_method: payment, items: cart.map((item) => ({ product_id: item.product.id, quantity: item.quantity, notes: item.notes })) }); localStorage.setItem('hookah-customer-last-order', JSON.stringify(cart)); setLastOrder(cart); if (created.checkout_url) { window.location.href = created.checkout_url; return } setLiveStatus(created.status); setOrder(created); setStep('success') } catch { setError('Não foi possível confirmar o pedido.') } finally { setSending(false) } }
