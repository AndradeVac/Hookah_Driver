import { ArrowLeft, ArrowRight, Check, ChevronRight, Clock, Heart, History, LoaderCircle, Minus, Plus, QrCode, RotateCcw, Search, ShoppingBag } from 'lucide-react'
...
// Fixed decorative banner (drop the file at frontend/public/images/banner-rosh.jpg).
const BANNER_IMAGE = '/images/banner-rosh.jpg'
const resolveImage = (url?: string | null) => (url && url.trim().length > 0 ? url : PLACEHOLDER_IMAGE)
...
export function CustomerJourneyPage() {
  ...
  useEffect(() => {
    ...
    if (!token) return
    getPublicOrder(token).then((tracking) => {
      setOrder({ order_id: token, order_number: tracking.order_number, status: tracking.status, total: tracking.total, public_token: token, payment_status: tracking.payment_status, checkout_url: null, pix_qr_code: tracking.pix_qr_code, pix_qr_code_base64: tracking.pix_qr_code_base64 })
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      const update = JSON.parse(event.data) as { status?: string; total?: string; payment_status?: string }
      if (!update.status) return
      setLiveStatus(update.status)
      setOrder((current) => current ? { ...current, status: update.status ?? current.status, total: update.total ?? current.total, payment_status: update.payment_status ?? current.payment_status } : current)
  ...
  {step === 'menu' && <section className="journey-menu"><label className="customer-search"><Search size={16} /><input placeholder="Buscar produtos" value={search} onChange={(event) => setSearch(event.target.value)} /></label><div className="journey-feature"><img src={BANNER_IMAGE} onError={onImageError} alt="Rosh com frutas" /><div><span>Mais pedidos</span><h2>Rosh</h2><p>Escolha uma marca e descubra os sabores.</p><button onClick={() => setStep('brands')}>Escolher Rosh <ArrowRight size={16} /></button></div></div>{history.length > 0 && <div className="journey-history"><h3>Pedidos anteriores</h3>{history.slice(0, 3).map((item) => <button key={item.order_number} onClick={() => repeatHistoryOrder(item)}><History size={16} /><span>Pedido #{item.order_number} · {money(item.total)}</span><RotateCcw size={15} /></button>)}</div>}{lastOrder.length > 0 && <button className="journey-reorder" onClick={() => { setCart(lastOrder); setStep('cart') }}><History size={16} /> Repetir último pedido <RotateCcw size={15} /></button>}<h3>Categorias</h3><div className="journey-categories">{categories.filter((item) => !['rosh', 'essências'].includes(item.name.toLowerCase())).map((category) => <button key={category.id} onClick={() => { setSelectedCategory(category); setStep('category') }}><img src={resolveImage(category.image_url)} onError={onImageError} alt="" /><span>{category.name}</span><ChevronRight size={16} /></button>)}</div></section>}
  ...
  return <main className="customer-app">{header}{error && <div className="customer-error">{error}</div>}
    ...
    {step === 'menu' && <section className="journey-menu"><label className="customer-search"><Search size={16} /><input placeholder="Buscar produtos" value={search} onChange={(event) => setSearch(event.target.value)} /></label><div className="journey-feature"><img src={BANNER_IMAGE} onError={onImageError} alt="Rosh com frutas" /><div><span>Mais pedidos</span><h2>Rosh</h2><p>Escolha uma marca e descubra os sabores.</p><button onClick={() => setStep('brands')}>Escolher Rosh <ArrowRight size={16} /></button></div></div>{lastOrder.length > 0 && <button className="journey-reorder" onClick={() => { setCart(lastOrder); setStep('cart') }}><History size={16} /><span>Repetir último pedido</span><RotateCcw size={15} /></button>}<h3>Categorias</h3><div className="journey-categories">{categories.filter((item) => !['rosh', 'essências'].includes(item.name.toLowerCase())).map((category) => <button key={category.id} onClick={() => { setSelectedCategory(category); setStep('category') }}><img src={resolveImage(category.image_url)} onError={onImageError} alt="" /><span>{category.name}</span><ChevronRight size={16} /></button>)}</div></section>}
    {step === 'category' && <section className="journey-panel"><button className="customer-back" onClick={back}><ArrowLeft size={16} /> Categorias</button><div className="journey-options">{filteredProducts.map((product) => <button key={product.id} onClick={() => { setSelectedProduct(product); setProductQuantity(1); setSelectedExtras([]); setStep('product') }}><img src={resolveImage(product.image_url)} onError={onImageError} alt="" /><span><strong>{product.name}</strong><small>{product.description || 'Preparado para você'}</small></span><strong>{money(product.price)}</strong><Heart className={favorites.includes(product.id) ? 'favorite-on' : ''} onClick={(event) => { event.stopPropagation(); toggleFavorite(product.id) }} size={16} /><ChevronRight size={17} /></button>)}</div></section>}
    {step === 'product' && selectedProduct && <section className="journey-panel journey-product-panel"><button className="customer-back" onClick={back}><ArrowLeft size={16} /> Voltar</button><img className="journey-product-image" src={resolveImage(selectedProduct.image_url)} onError={onImageError} alt={selectedProduct.name} /><span className="customer-product-tag">{selectedCategory?.name}</span><h2>{selectedProduct.name}</h2><p>{selectedProduct.description || 'Preparado para você'}</p><textarea className="customer-note" placeholder="Observação para este item (opcional)" value={productNotes} onChange={(event) => setProductNotes(event.target.value)} maxLength={300} /> <div className="journey-quantity"><span>Quantidade</span><button onClick={() => setProductQuantity((value) => Math.max(1, value - 1))}><Minus size={15} /></button><strong>{productQuantity}</strong><button onClick={() => setProductQuantity((value) => value + 1)}><Plus size={15} /></button></div>{extraProducts.length > 0 && <><h3>Adicionais</h3><div className="journey-extras">{extraProducts.slice(0, 6).map((extra) => <button className={selectedExtras.some((item) => item.id === extra.id) ? 'selected' : ''} key={extra.id} onClick={() => setSelectedExtras((current) => current.some((item) => item.id === extra.id) ? current.filter((item) => item.id !== extra.id) : [...current, extra])}><img src={resolveImage(extra.image_url)} onError={onImageError} alt="" /><span>{extra.name}</span><b>{money(extra.price)}</b></button>)}</div></>}<button className="customer-primary" onClick={() => { add(selectedProduct, undefined, productNotes); for (let index = 1; index < productQuantity; index += 1) add(selectedProduct, undefined, productNotes); selectedExtras.forEach((extra) => add(extra)); setStep('cart') }}>Adicionar ao pedido <Plus size={16} /></button></section>}
    {(step === 'brands' || step === 'flavors' || step === 'rosh') && <section className="journey-panel"><button className="customer-back" onClick={back}><ArrowLeft size={16} /> Voltar</button>{step === 'brands' && <div className="journey-options">{brands.map((item) => <button key={item.id} onClick={() => { setBrand(item); setStep('flavors') }}><img src={resolveImage(item.image_url)} onError={onImageError} alt="" /><span><strong>{item.name}</strong><small>{flavors.filter((itemFlavor) => itemFlavor.brand_id === item.id).length} sabores disponíveis</small></span><ChevronRight size={17} /></button>)}</div>}{step === 'flavors' && <div className="journey-options">{brandFlavors.map((item) => <button key={item.id} onClick={() => { setFlavor(item); setStep('rosh') }}><img src={resolveImage(item.image_url)} onError={onImageError} alt="" /><span><strong>{item.name}</strong><small>{brand?.name}</small></span><ChevronRight size={17} /></button>)}</div>}{step === 'rosh' && rosh && flavor && <article className="journey-rosh-card"><img src={resolveImage(flavor.image_url)} onError={onImageError} alt="Rosh" /><span>{brand?.name}</span><h2>Rosh</h2><p>Sabor {flavor.name}</p><strong>{money(rosh.price)}</strong><button onClick={() => { add(rosh, flavor.name); setStep('cart') }}>Adicionar ao pedido <Plus size={16} /></button></article>}</section>}
    {step === 'cart' && <section className="customer-panel"><button className="customer-back" onClick={() => setStep('menu')}><ArrowLeft size={16} /> Menu</button><h2>Resumo do pedido</h2>{cart.length ? cart.map((item) => <div className="customer-cart-row" key={`${item.product.id}-${item.variation}`}><img src={resolveImage(item.product.image_url)} onError={onImageError} alt="" /><div><strong>{item.product.name}</strong><span>{item.variation || ''}{item.notes ? ` · ${item.notes}` : ''}</span></div><div><button onClick={() => change(item, -1)}><Minus size={14} /></button><b>{item.quantity}</b><button onClick={() => change(item, 1)}><Plus size={14} /></button></div></div>) : <div className="customer-empty">Seu pedido está vazio.</div>}<div className="customer-total">Total <strong>{money(total)}</strong></div><button className="customer-primary" disabled={!cart.length} onClick={() => setStep('customer')}>Confirmar pedido <ArrowRight size={16} /></button></section>}
