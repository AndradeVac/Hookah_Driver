...
type Step = 'register' | 'menu' | 'brands' | 'flavors' | 'rosh' | 'cart' | 'customer' | 'payment' | 'success'
...
export function CustomerJourneyPage() {
  ...
  const back = () => setStep(step === 'brands' ? 'menu' : step === 'flavors' ? 'brands' : step === 'rosh' ? 'flavors' : step === 'cart' ? 'menu' : 'menu')
  ...
  const headerTitle = { register: 'Antes de começar', menu: 'Cardápio', brands: 'Marcas', flavors: 'Sabores', rosh: 'Seu Rosh', cart: 'Seu pedido', customer: 'Confirmar pedido', payment: 'Pagamento', success: 'Pedido confirmado' }[step]
